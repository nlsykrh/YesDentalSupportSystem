...
public class BillingDAO {
    ...
    // Get billing by ID
// Get billing by ID with installment data
public Billing getBillingById(String billingId) {
    Billing billing = null;
    // Include patient name in the query
    String sql = "SELECT b.*, a.appointment_date, a.patient_ic, p.patient_name " +
                "FROM billing b " +
                "JOIN appointment a ON b.appointment_id = a.appointment_id " +
                "LEFT JOIN patient p ON a.patient_ic = p.patient_ic " +
                "WHERE b.billing_id = ?";
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        if (rs.next()) {
            ...
            // Get installment dates if billing method is Installment
            // In getBillingById method, update the installment dates retrieval section:

if ("Installment".equalsIgnoreCase(billing.getBillingMethod())) {
    List<Installment> installments = getInstallmentsByBillingId(billingId);
    
    if (installments != null && !installments.isEmpty()) {
        // Set number of installments from first installment
        billing.setNumInstallments(installments.get(0).getNumofinstallment());
        
        // Sort installments by payment date
        Collections.sort(installments, new Comparator<Installment>() {
            @Override
            public int compare(Installment i1, Installment i2) {
                try {
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                    Date date1 = (Date) sdf.parse(i1.getPaymentDate());
                    Date date2 = (Date) sdf.parse(i2.getPaymentDate());
        ...
        // Extract and concatenate payment dates
        StringBuilder dates = new StringBuilder();
        for (int i = 0; i < installments.size(); i++) {
            Installment installment = installments.get(i);
            if (installment.getPaymentDate() != null && 
                !installment.getPaymentDate().trim().isEmpty()) {
                if (i > 0) {
                    dates.append(",");
                }
                dates.append(installment.getPaymentDate().trim());
            }
        }
        
        // Set the installment dates string
        billing.setInstallmentDates(dates.toString());
        
        // Debug logging
        System.out.println("DEBUG: Found " + installments.size() + " installments for billing " + billingId);
        System.out.println("DEBUG: Installment dates string: " + dates.toString());
    } else {
        billing.setNumInstallments(0);
        billing.setInstallmentDates(null);
    }
} else {
    billing.setNumInstallments(0);
    billing.setInstallmentDates(null);
...
private Billing getBillingByIdWithinTransaction(Connection conn, String billingId) throws SQLException {
    ...
    // Add ORDER BY to ensure proper sorting
    String sql = "SELECT * FROM installment WHERE billing_id = ? ORDER BY payment_date ASC";
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            System.out.println("Installment ID: " + installment.getInstallmentId() + 
                             ", Date: " + installment.getPaymentDate());
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
...
//        }
//    }
    // Get installment by installment ID
public List<Installment> getInstallmentsByInstallmentId(String installmentId) {
    List<Installment> installments = new ArrayList<>();
