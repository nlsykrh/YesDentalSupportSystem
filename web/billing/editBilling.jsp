...
<% if (billing != null) { 
    ...
    List<String> existingInstallmentDates = new ArrayList<String>();
    int existingNumInstallments = 0;
    
    if (billing.getInstallmentDates() != null && !billing.getInstallmentDates().isEmpty()) {
        String[] dates = billing.getInstallmentDates().split(",");
        existingInstallmentDates = Arrays.asList(dates);
        existingNumInstallments = billing.getNumInstallments() > 0 ? 
            billing.getNumInstallments() : dates.length;
    }
%>
                ...
                <form action="${pageContext.request.contextPath}/BillingServlet" method="post" id="billingForm">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <!-- Installment Section -->
                            <div class="row" id="installmentSection" style="display: none;">
                                <div class="col-md-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <select class="form-control" name="num_installments" id="numInstallments">
                                                    <% for (int i = 1; i <= 5; i++) { %>
                                                        <option value="<%= i %>" <%= (existingNumInstallments == i) ? "selected" : "" %>>
                                                            <%= i %> Installment<%= i > 1 ? "s" : "" %>
                                                        </option>
                                                    <% } %>
                                                </select>
       ...
       value="<%= billing != null && billing.getInstallmentDates() != null ? 
               billing.getInstallmentDates() : "" %>">
...
<script>
    ...
    function generateDateInputs() {
        ...
        // Get existing dates from hidden field
        const existingDatesStr = document.getElementById('installmentDates').value;
        const existingDates = existingDatesStr ? existingDatesStr.split(',') : [];
        
        for (let i = 1; i <= numInstallments; i++) {
            // Use existing date if available, otherwise empty
            const existingDate = (i - 1) < existingDates.length ? existingDates[i - 1] : '';
...
function initializeFlatpickr() {
    document.querySelectorAll('.installment-date').forEach(function(input) {
        flatpickr(input, {
            dateFormat: "Y-m-d",
            minDate: "today"
        });
...
function generateDateInputs() {
    ...
    // Get existing dates from the billing object passed from server
    const existingDatesStr = '<%= billing != null && billing.getInstallmentDates() != null ? 
        billing.getInstallmentDates() : "" %>';
    const existingDates = existingDatesStr ? existingDatesStr.split(',') : [];
    
    for (let i = 1; i <= numInstallments; i++) {
        // Use existing date if available, otherwise empty
        const existingDate = (i - 1) < existingDates.length ? existingDates[i - 1].trim() : '';
    ...
    // Re-initialize flatpickr for new inputs
    setTimeout(() => {
        initializeFlatpickr();
        updateInstallmentDates();
    }, 100);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check the current billing method
    const billingMethod = document.getElementById('billingMethod');
    const installmentSection = document.getElementById('installmentSection');
    
    // Only show installment section if user selects it from dropdown
    // Initially hide it even if db value is "Installment"
    installmentSection.style.display = 'none';
